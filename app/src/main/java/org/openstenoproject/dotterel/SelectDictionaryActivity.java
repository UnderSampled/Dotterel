...
public class SelectDictionaryActivity extends ListActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(org.openstenoproject.dotterel.R.layout.dictionary_list);
        loadDictionaryList();
        adapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, list);
        Button addButton = new Button(this);
        addButton.setText(getString(org.openstenoproject.dotterel.R.string.add_dictionary));
    ...
    private void loadDictionaryList() {
        String dictionaries = prefs.getString(getString(org.openstenoproject.dotterel.R.string.key_dictionaries), "");
        for (String dictionary : dictionaries.split(DotterelApp.DELIMITER)) {
    ...
    private void updatePreference() {
        ...
        for (String d : pathList) {
            if (!d.trim().isEmpty()) {
                dictionaries += DotterelApp.DELIMITER + d;
            }
        }
        if (!dictionaries.isEmpty()) { //list is prefixed with DELIMITER
            dictionaries = dictionaries.substring(1);
        }
        prefs.edit().putString(getString(org.openstenoproject.dotterel.R.string.key_dictionaries), dictionaries).commit();
    }
